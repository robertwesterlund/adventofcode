...
type Number =
    { Row: int32
      ...
      EndsAt: int32
      Number: int64
      BlockAround: Block seq
      GearSymbolLocations: (int32 * int32) seq }

let getNumbers (input: string array) =
    let mutable numbers = []
    ...
    |> Seq.iteri (fun index line ->
        ...
        while start < length do
            if Char.IsDigit(lineSpan.[start]) then
                let mutable endsBefore = start + 1

                while endsBefore < length
                      && Char.IsDigit(line.[endsBefore]) do
                    endsBefore <- endsBefore + 1

                let blockAround =
                    [ max (index - 1) 0
                      index
                      min (index + 1) (input.Length - 1) ]
                    |> List.distinct
                    |> Seq.map (fun blockRow ->
                        let blockStart = max 0 (start - 1)

                        let blockLength =
                            min (endsBefore + 1 - blockStart) (input.[blockRow].Length - blockStart)

                        { RowIndex = blockRow
                          StartAt = blockStart
                          Text =
                            input.[blockRow]
                                .Substring(blockStart, blockLength) })
                    |> Seq.toArray

                let gearSymbolLocations =
                    blockAround
                    |> Seq.collect (fun block ->
                        block.Text
                        |> Seq.mapi (fun i c ->
                            let position =
                                (((int32) i) + block.StartAt), block.RowIndex
                ...
                numbers <-
                    [ { Number = Int64.Parse(lineSpan.Slice(start, endsBefore - start))
                        EndsAt = endsBefore - 1
                        StartsAt = start
                        Row = index
                        BlockAround = blockAround
                        GearSymbolLocations = (gearSymbolLocations |> Array.toSeq) } ]
                    |> List.append numbers
...
let hasAdjacentSymbol number =
    let mutable symbols = []

    for row in number.BlockAround do
        for i in 0 .. (row.Text.Length - 1) do
            let c = row.Text.[i]

            if (not (Char.IsDigit(c)) && (not (c.Equals('.')))) then
                symbols <- [ c ] |> List.append symbols

    symbols.Length > 0

let getGearRatio numbers =
    numbers
    |> Seq.collect (fun n -> n.GearSymbolLocations |> Seq.map (fun g -> (g, n)))
    |> Seq.groupBy (fun (gearPosition, number) -> gearPosition)
    |> Seq.filter (fun (position, group) -> group |> Seq.length = 2)
    |> Seq.map (fun (_, group) ->
        let arr =
            group |> Seq.map (fun (_, n) -> n) |> Seq.toArray

        arr.[0].Number * arr.[1].Number)

testData
|> getNumbers
|> Seq.filter hasAdjacentSymbol
|> Seq.sumBy (fun n -> n.Number)
