...
type Game = { Id: int64; Sets: Set seq }

let setRegex =
    new Regex(
        """(?:(?<green>\d+) green|(?<blue>\d+) blue|(?<red>\d+) red){1,3}""",
        RegexOptions.Compiled
        ||| RegexOptions.ExplicitCapture
    )

let getValueOrDefaultFromGroup (captureGroupName: string) (groupCollection: GroupCollection) =
    let group = groupCollection.Item(captureGroupName)

    if group.Success then
        Int64.Parse(group.Value)
    else
        0
...
let parse (inputString: string) =
    ...
    let games =
        rows
        |> Seq.map (fun line ->
            let gameSetSplit = line.Split([| ':' |])
            let setStrings = gameSetSplit.[1].Split([| ';' |])

            let sets =
                setStrings
                |> Seq.map (fun setString ->
                    let matches = setRegex.Matches(setString)
                    ...
                    |> Seq.fold
                        (fun a c ->
                            { Red =
                                a.Red
                                + (c.Groups |> getValueOrDefaultFromGroup "red")
                              Green =
                                a.Green
                                + (c.Groups |> getValueOrDefaultFromGroup "green")
                              Blue =
                                a.Blue
                                + (c.Groups |> getValueOrDefaultFromGroup "blue") })
                        { Red = 0; Green = 0; Blue = 0 })

            { Id = Int64.Parse(gameSetSplit.[0].Substring("Game ".Length))
              Sets = sets })

    games

let getPossibleGamesInPart1 games =
    games
    |> Seq.filter (fun g ->
        not (
            g.Sets
            |> Seq.exists (fun s -> s.Red > 12 || s.Green > 13 || s.Blue > 14)
        ))

let getLeastNumberOfCubes games =
    games
    |> Seq.map (fun g ->
        g.Sets
        |> Seq.fold
            (fun a c ->
                { Red = max a.Red c.Red
                  Green = max a.Green c.Green
                  Blue = max a.Blue c.Blue })
            { Red = 0; Green = 0; Blue = 0 })

let countPowerOfCubes cubeSets =
    cubeSets
    |> Seq.sumBy (fun s -> s.Red * s.Green * s.Blue)

let part1 input =
    input
    |> parse
    |> getPossibleGamesInPart1
    |> Seq.sumBy (fun g -> g.Id)

let part2 input =
    input
    |> parse
    |> getLeastNumberOfCubes
    |> countPowerOfCubes

sampleData
|> part1
|> printfn "Test data, part 1: %i"

sampleData
|> part2
|> printfn "Test data, part 2: %i"
