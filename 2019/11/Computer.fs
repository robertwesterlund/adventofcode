...
type Context with
    ...
    member this.WithDataEntryReplaced (indexParameter: Parameter) newValue =
        let index =
            ...
            | Immediate _ ->
                failwith "Shouldn't end up with an immediate parameter for location of where to store data"
        { this with
              Data =
    ...
    static member CreateFromDataArray data =
        let convertToMap arr = new Map<int32, int64>(arr |> Array.mapi (fun index value -> (index, value)))
        data
        |> convertToMap
        |> Context.Create
...
type OpCode =
    ...
    member this.Apply(context: Context) =
        ...
        | JumpIfFalse(p1, p2) ->
            ...
            | _ -> context
        | LessThan(l, r, o) ->
            context.WithDataEntryReplaced o
        ...
        | AdjustRelativeBase offset ->
            { context with RelativeBaseOffset = context.RelativeBaseOffset + int32 (offset.GetValue(context)) }
...
let getCurrentOperation context =
    ...
    let getParameter paramIndex: Parameter =
        ...
        | unknown ->
            failwith
                ("Unknown parameter mode " + unknown.ToString() + " at index " + context.InstructionPointer.ToString()
                 + ". Instruction was: " + instruction.ToString())
    ...
    | unknown ->
        failwith
            ("Unknown operation requested: " + unknown.ToString() + ", at index "
             + context.InstructionPointer.ToString() + ". Instruction was: " + instruction.ToString())
